...
function assertValidISODateYYYYMMDD(s) {
    ...
    if (m < 1 || m > 12)
        throw new Error(`Invalid birthDate month: ${s}`);
    if (d < 1 || d > 31)
        throw new Error(`Invalid birthDate day: ${s}`);
...
function dayOfYearUTC(dateUTC) {
    ...
    const cur = Date.UTC(dateUTC.getUTCFullYear(), dateUTC.getUTCMonth(), dateUTC.getUTCDate());
...
function buildVerdict(score, locale) {
    if (locale === "en") {
        if (score >= 67)
            return "Silný signál. Základ pôsobí nezvyčajne stabilne.";
        if (score >= 34)
            return "Zmiešaný signál. Niečo sedí, niečo škrípe.";
        return "Slabý signál. Rátaj s výkyvmi a korekciami.";
    }
    if (locale === "cz") {
        if (score >= 67)
            return "Vysoký signál. Základ vyzerá nezvyčajne stabilne.";
        if (score >= 34)
            return "Zmiešaný signál. Niečo sedí, niečo drhne.";
...
function buildFacts(subject, birthDate, nameNormalized, locale, score, seedHash) {
...
export function compute(input) {
    ...
    return {
        ...
        facts: buildFacts(subject, input.birthDate, nameNormalized, locale, score, seedHash),
        meta: {
